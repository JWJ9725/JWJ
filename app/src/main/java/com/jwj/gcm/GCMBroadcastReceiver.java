...
public class GCMBroadcastReceiver extends WakefulBroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {        //상대방이 메시지 보낼때  intent의 부가적인 정보로 사용
        ...
        if (action != null) {
            if (action.equals("com.google.android.c2dm.intent.RECEIVE")) { // 푸시 메시지 수신 시
                ...
                sendToNotification(context, from, command, type, data);
    ...
    private void sendToNotification(Context context, String from, String command, String type, String data) {
        ...
        NotificationCompat.Builder mBuilder =
                new NotificationCompat.Builder(context).setSmallIcon(R.drawable.cheese_1).setContentTitle("GCM Notification").setStyle(new NotificationCompat.BigTextStyle().bigText(data)).setContentText(data);
